...
/*
		This function reads in the binary file format of 8^3 leaf tiles by vassvik
*/
function get16bitVDBData(path) {
	...
	download(path, "arrayBuffer").then((data) => {
		...
		let cloudData = new Uint16Array(resolution[0] * resolution[1] * resolution[2]);
		/*
				console.log("Dimensions: ", dimensions);
				console.log("Resolution: ", resolution);
				console.log("Format: ", header.getUint16(6, true));
				console.log("Sparse: ", header.getUint16(8, true));
				console.log("Block size: ", header.getUint16(10, true));
		*/
		let nodeCount = header.getUint32(12, true);
		/*
				console.log("Tiles: ", nodeCount);
				console.log("Normalization: ", header.getFloat32(16, true));
				console.log("Offsets: ", header.getInt32(20, true),  header.getInt32(24, true),  header.getInt32(28, true));
		*/

		const coordinates = new Int32Array(data, 32, 3 * nodeCount);
		const nodeData = new Uint16Array(data, 32 + 4 * 3 * nodeCount, 8 * 8 * 8 * nodeCount);
		...
		let compressedData = new Uint8Array(resolution[0] * resolution[1] * resolution[2]);

		for (let i = 0; i < cloudData.length; i++) {
			if (cloudData[i] > 0) {
				let a = cloudData[i] / 65535;
				let x = 255 * a;
				compressedData[i] = x;
			}
		}
		...
		gl.activeTexture(gl.TEXTURE0);
		gl.bindTexture(gl.TEXTURE_3D, vdbTexture);
		gl.texImage3D(gl.TEXTURE_3D, 0, gl.R8, resolution[0], resolution[1], resolution[2], 0, gl.RED, gl.UNSIGNED_BYTE, compressedData);
	});
